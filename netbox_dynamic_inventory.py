...
group_by = ["sites", "tenant", "enclave", "deployment"]
...
devices_get = requests.get(
    netbox_url + devices_api, headers=header, verify=False
).json()
interfaces_get = requests.get(
    netbox_url + interfaces_api, headers=header, verify=False
).json()
ip_addresses_get = requests.get(
    netbox_url + ip_addresses_api, headers=header, verify=False
).json()
platforms_get = requests.get(
    netbox_url + platforms_api, headers=header, verify=False
).json()
prefixes_get = requests.get(
    netbox_url + prefixes_api, headers=header, verify=False
).json()
...
# import sys
# f = open('netbox_inventory_output.txt', mode='wt', encoding='utf-8')
# f.write(json.dumps(overall_list, indent=4, sort_keys=True))